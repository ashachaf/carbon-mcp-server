    ...
    async (uri) => ({
      contents: [{
        ...
        text: [
          ...
          "- If below market only: set buy_price_marginal to current market price",
          "- When sell budget is 0 on recurring strategy: inform user sell capacity is pre-calculated from buy marginal price",
          "",
          "## Warnings to Surface",
          "- Buy price above market price: warn and offer to adjust",
          "- Sell price below market price: warn and offer to adjust",
          "- Overlapping buy/sell ranges: warn but allow - ask user to confirm",
          "- Insufficient token allowance: show approval steps before transaction",
          "",
          "## Strategy Types",
          "- carbon_create_limit_order: single exact price, one-time",
          "- carbon_create_range_order: gradual execution across price range, one-time",
          "- carbon_create_recurring_strategy: buy low + sell high loop, indefinite, zero gas on fills",
          "- carbon_create_concentrated_strategy: two-sided liquidity with spread",
          "- carbon_get_strategies: always call first",
          "- carbon_delete_strategy: irreversible - confirm with user first",
        ].join("\n"),
      }],
    })
  );
